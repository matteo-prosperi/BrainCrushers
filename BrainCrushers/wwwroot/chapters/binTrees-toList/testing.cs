...
{
    protected override (string ToPrint, bool Success) Run(BrainCrushers.Exercise.Node? root)
    {
        ...
        var result = exercise.TreeToLists(root);

        bool success = (result.PreOrder?.SequenceEqual(expectedPreOrder) ?? false) &&
                       (result.InOrder?.SequenceEqual(expectedInOrder) ?? false) &&
                       (result.PostOrder?.SequenceEqual(expectedPostOrder) ?? false);
        return ($"PreOrder: {PrintArray(result.PreOrder)}{Environment.NewLine}InOrder: {PrintArray(result.InOrder)}{Environment.NewLine}PostOrder: {PrintArray(result.PostOrder)}", success);
    }
}
...
public class Tester2 : BaseTester
{
    protected override (string ToPrint, bool Success) Run(BrainCrushers.Exercise.Node? root)
    {
        ...
        BrainCrushers.Exercise exercise = new();
        var result = exercise.BreadthFirstToList(root);

        bool success = result?.SequenceEqual(expectedResult) ?? false;
        return ($"{PrintArray(result)}", success);
    }
...
public abstract class BaseTester
{
    ...
    protected abstract (string ToPrint, bool Success) Run(BrainCrushers.Exercise.Node? root);
    ...
    public async IAsyncEnumerable<string> TestAsync()
    {
           ...
           })
        {
            ...
            (string ToPrint, bool Success) result;
            try
            {
                result = Run(root);
            }
            ...
            yield return $"{result.ToPrint} {(result.Success ? '✓' : '✗')}{Environment.NewLine}{Environment.NewLine}";
            if (!result.Success)
            {
